...
int main ()
{
  ...
  while((row = mysql_fetch_row(confres)))
  {
    for(int i = 0; i < numfields; i++)
    {
      ...
      if (i == 1)
      {
        mysql_query(conn,concat("SELECT CONCAT(rede_subrede,' netmask ',netmask_subrede) AS chave, cod_subrede from subrede WHERE ativo_subrede = 1 AND cod_areacmts =", val));
